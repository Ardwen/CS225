...
class BinaryTree
{
    ...
    private:
      void getPaths(Node* cur, vector<T>path, vector<vector<T>>& paths ) const;
      void mirror(Node* root);
      bool isOrderedRecursive(Node* current) const;
      vector<T> inO(BinaryTree::Node* subRoot, std::vector<T>& treeVector) const;
      int max(Node* current) const;
