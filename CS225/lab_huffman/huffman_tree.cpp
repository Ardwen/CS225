                            ...
                            queue<TreeNode*>& mergeQueue)
{
  if (singleQueue.size() == 0) {
    TreeNode * returnnode = mergeQueue.front();
    mergeQueue.pop();
    return returnnode;
  }
  if (mergeQueue.size() == 0) {
    TreeNode * returnnode = singleQueue.front();
    singleQueue.pop();
    return returnnode;
  }
  Frequency first = singleQueue.front() -> freq;
  Frequency second =  mergeQueue.front() -> freq;
  TreeNode* small;
  if (first < second) {
    small = singleQueue.front();
    singleQueue.pop();
  } else if (first == second) {
    small = singleQueue.front();
    singleQueue.pop();
  } else {
    small = mergeQueue.front();
    mergeQueue.pop();
  }
  return small;  /**
     * @todo Your code here!
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
      ...
      TreeNode *newnode = new TreeNode(first -> freq.getFrequency() +
        second -> freq.getFrequency());
      newnode -> left = first;
      newnode -> right = second;
      mergeQueue.push(newnode);
    }
    TreeNode *single = mergeQueue.front();
    root_ = single;
    mergeQueue.pop();
    /**
     * @todo Your code here!
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  ...
  if (bfile.getNextBit()) {
    Frequency *newf = new Frequency(bfile.getNextByte(),0);
    TreeNode *newnode = new TreeNode(*newf);
    return newnode;
  } else {
